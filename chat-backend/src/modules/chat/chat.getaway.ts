...
//使用 @WebSocketGateway 装饰器，将一个普通的类转换成一个 WebSocket ，
//该服务器可以处理客户端的连接、断开连接和消息发送等事件。
//使得能够在类中定义与 WebSocket 通信相关的逻辑，
@WebSocketGateway({
  path: '/chat',
  allowEIO3: true,
  cors: {
...
export class WsChatGateway {
  ...
  //...Server装饰器装饰，将socket标记为WebSocket服务器实例
  ...
  async handleDisconnect(client: Socket) {
    ...
    /* 如果这个用户离开后房间没人了 那么我们关闭这个房间以及定时器 如果还剩人那么通知房间所有人更新在线用户列表 */
  ...
  /* 接收到客户端的消息 */
  //此装饰器表示监听客户端发送过来的名为'message'的事件
  ...
  async handleMessage(client: Socket, data: any) {
   ...
   console.log(user_tags)
    ...
    // 将处理结果通过 socket 连接发送到指定房间的所有客户端
    this.socket
      .to(room_id)
  ...
  /**
   * @desc 用户在客户端修改信息后应该通知房间变更用户信息，否则新的聊天的头像名称依然是老的用户信息
   * @param client
